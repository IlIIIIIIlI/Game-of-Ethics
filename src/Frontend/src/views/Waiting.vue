    ...
    <v-col class="text-center">
      ...
      <div class="my-6">
        <v-card class="mx-auto" tile max-width="400">
          <v-list-item v-for="(player, index) in players" :key="index"> <!--  Traversing the players list by v-for -->
            <v-list-item-content>
      ...
      <div class="my-6">
        <p>Your Name: {{ this.playerRole }}</p> <!-- Display Current Player Identity -->
      </div>
    ...
    return axios
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            if (!vm.playerRole) { // If the current player identity has not been set
              vm.playerRole = response.data.players[response.data.players.length - 1]; // Get the last position in the list as the current player identity
            }
          }
        })
        .catch((error) => {
...
export default {
  data() {
    return {
      ...
      playerRole: "", // Current Player Identity
    };
  },
  created() {
    this.updatePlayers();
